...
import type {
  PurchasingGroupListFilters,
  PurchasingGroupListResult,
} from "../types/purchasing-group.type";

const purchasingGroupListSelect = {
  id: true,
  code: true,
  name: true,
  description: true,
...
export async function purchasingGroupListRepository(
  filters: PurchasingGroupListFilters,
): Promise<PurchasingGroupListResult> {
  ...
  const [rows, total] = await Promise.all([
    prisma.purchasingGroup.findMany({
      where,
      select: purchasingGroupListSelect,
