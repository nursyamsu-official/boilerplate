"use client";

import { useCallback, useMemo } from "react";
import type { SortingState } from "@tanstack/react-table";

import { DataTable } from "@/components/data-table/DataTable";
import { TablePagination } from "@/components/data-table/TablePagination";
import type { CountryOption } from "@/features/countries";

import type { ProvinceFilterInput } from "../schemas/province-filter.schema";
import type {
  ProvinceListResult,
  ProvinceTableRow,
} from "../types/province.type";
import { createProvinceColumns } from "./columns";
import { ProvinceTableToolbar } from "./ProvinceTableToolbar";

type ProvinceTableProps = {
  data: ProvinceListResult;
  filters: ProvinceFilterInput;
  countryOptions: CountryOption[];
  isEditLoading?: boolean;
  onFiltersChange: (filters: Partial<ProvinceFilterInput>) => void;
  onEdit: (province: ProvinceTableRow) => void;
  onCreate: () => void;
  onRefresh: () => void;
};

export function ProvinceTable({
  data,
  filters,
  countryOptions,
  isEditLoading = false,
  onFiltersChange,
  onEdit,
  onCreate,
  onRefresh,
}: ProvinceTableProps) {
  const sorting: SortingState = useMemo(
    () => [{ id: filters.sortBy, desc: filters.sortOrder === "desc" }],
    [filters.sortBy, filters.sortOrder],
  );

  const handleEdit = useCallback(
    (province: ProvinceTableRow) => {
      if (isEditLoading) return;
      void onEdit(province);
    },
    [isEditLoading, onEdit],
  );

  const columns = useMemo(
    () =>
      createProvinceColumns({
        sortBy: filters.sortBy,
        sortOrder: filters.sortOrder,
        onSortChange: (sortBy) => {
          const nextSortOrder =
            filters.sortBy === sortBy && filters.sortOrder === "asc"
              ? "desc"
              : "asc";

          onFiltersChange({
            sortBy: sortBy as ProvinceFilterInput["sortBy"],
            sortOrder: nextSortOrder,
            page: 1,
          });
        },
        onEdit: handleEdit,
        onRefresh,
      }),
    [filters.sortBy, filters.sortOrder, handleEdit, onRefresh, onFiltersChange],
  );

  const isFiltered =
    filters.search.length > 0 ||
    filters.isActive !== "all" ||
    filters.countryId !== "all";

  return (
    <div className="flex flex-col gap-4">
      <ProvinceTableToolbar
        filters={filters}
        countryOptions={countryOptions}
        onFiltersChange={onFiltersChange}
        onCreate={onCreate}
      />

      <DataTable
        columns={columns}
        data={data.items}
        sorting={sorting}
        isFiltered={isFiltered}
        emptyTitle="No provinces yet"
        emptyDescription="Create your first province to get started."
        onSortingChange={(nextSorting) => {
          const first = nextSorting[0];
          if (!first) return;

          onFiltersChange({
            sortBy: first.id as ProvinceFilterInput["sortBy"],
            sortOrder: first.desc ? "desc" : "asc",
            page: 1,
          });
        }}
      />

      <TablePagination
        page={data.page}
        pageSize={data.pageSize}
        total={data.total}
        onPageChange={(page) => onFiltersChange({ page })}
        onPageSizeChange={(pageSize) =>
          onFiltersChange({ pageSize, page: 1 })
        }
      />
    </div>
  );
}
