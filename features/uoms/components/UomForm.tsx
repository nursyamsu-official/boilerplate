"use client";

import { useForm } from "@tanstack/react-form";

import { FormSubmitError } from "@/components/form/FormSubmitError";
import { FormShell, getFormClassName } from "@/components/form/form-dialog-layout";
import { NumberField } from "@/components/form/NumberField";
import { SelectField } from "@/components/form/SelectField";
import { SwitchField } from "@/components/form/SwitchField";
import { TextField } from "@/components/form/TextField";
import { TextareaField } from "@/components/form/TextareaField";
import { Button } from "@/components/ui/button";
import { FieldGroup } from "@/components/ui/field";
import { Spinner } from "@/components/ui/spinner";
import {
  useFormActionSubmit,
  type FormActionSubmitConfig,
} from "@/lib/use-form-action-submit";

import {
  UOM_TYPE_LABELS,
  UOM_TYPE_VALUES,
} from "../constants/uom.constants";
import type { UomFormValues } from "../types/uom.type";

const uomTypeOptions = UOM_TYPE_VALUES.map((value) => ({
  value,
  label: UOM_TYPE_LABELS[value],
}));

type UomFormProps = {
  defaultValues: UomFormValues;
  submitLabel: string;
  pendingLabel: string;
  layout?: "default" | "dialog";
  submitConfig: FormActionSubmitConfig<UomFormValues, unknown>;
  onCancel: () => void;
};

export function UomForm({
  defaultValues,
  submitLabel,
  pendingLabel,
  layout = "default",
  submitConfig,
  onCancel,
}: UomFormProps) {
  const { onSubmitAsync, onSubmit } = useFormActionSubmit(submitConfig);

  const form = useForm({
    defaultValues,
    validators: {
      onSubmitAsync,
    },
    onSubmit: async () => {
      await onSubmit();
    },
  });

  const formActions = (
    <>
      <Button type="button" variant="outline" onClick={onCancel}>
        Cancel
      </Button>
      <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
        {([canSubmit, isSubmitting]) => (
          <Button type="submit" disabled={!canSubmit || isSubmitting}>
            {isSubmitting ? (
              <>
                <Spinner className="size-4" />
                {pendingLabel}
              </>
            ) : (
              submitLabel
            )}
          </Button>
        )}
      </form.Subscribe>
    </>
  );

  return (
    <form
      className={getFormClassName(layout)}
      onSubmit={(event) => {
        event.preventDefault();
        event.stopPropagation();
        void form.handleSubmit();
      }}
    >
      <FormShell layout={layout} actions={formActions}>
        <FieldGroup>
          <FormSubmitError form={form as never} />
          <form.Field name="code">
            {(field) => (
              <TextField
                field={field}
                label="Code"
                description="Lowercase letters, numbers, and underscores only."
                placeholder="kg"
                required
              />
            )}
          </form.Field>

          <form.Field name="name">
            {(field) => (
              <TextField
                field={field}
                label="Name"
                placeholder="Kilogram"
                required
              />
            )}
          </form.Field>

          <form.Field name="symbol">
            {(field) => (
              <TextField
                field={field}
                label="Symbol"
                placeholder="kg"
                description="Optional display symbol."
              />
            )}
          </form.Field>

          <form.Field name="uomType">
            {(field) => (
              <SelectField
                field={field}
                label="Type"
                options={uomTypeOptions}
                placeholder="Select UOM type"
                required
              />
            )}
          </form.Field>

          <form.Field name="decimalPlaces">
            {(field) => (
              <NumberField
                field={field}
                label="Decimal places"
                description="Number of decimal places for quantities (0–6)."
                min={0}
                required
              />
            )}
          </form.Field>

          <form.Field name="description">
            {(field) => (
              <TextareaField
                field={field}
                label="Description"
                placeholder="Brief description of the unit"
              />
            )}
          </form.Field>

          <form.Field name="isActive">
            {(field) => (
              <SwitchField
                field={field}
                label="Active"
                description="Inactive UOMs are hidden from selection lists."
              />
            )}
          </form.Field>
        </FieldGroup>
      </FormShell>
    </form>
  );
}
